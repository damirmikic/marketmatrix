// Futsal API Module
// Handles fetching futsal matches and odds from Kambi API
...
function buildHierarchy(events) {
    ...
    events.forEach(event => {
        ...
        futsalData.eventMap[event.event.id] = event;
    });

    // Convert map to array and sort
    futsalData.leagues = Array.from(leagueMap.values())
        .sort((a, b) => a.name.localeCompare(b.name));
}

// Fetch futsal matches from list view endpoint
export async function initLoader() {
    try {
        ...
        if (data.events && data.events.length > 0) {
            ...
            console.warn('No futsal matches available');
        }
    } catch (error) {
...
function populateLeagueSelector() {
    ...
    futsalData.leagues.forEach(league => {
...
export function handleLeagueChange() {
    ...
    const league = futsalData.leagues.find(l => l.name === leagueName);
    if (!league) return;
