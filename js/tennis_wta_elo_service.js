...
class TennisWtaEloService {
    ...
    /**
     * Fetch current WTA Elo ratings from Tennis Abstract
    ...
    async fetchEloRatings() {
        try {
            ...
            if (!table) {
                throw new Error('Could not find WTA Elo ratings table');
            }
            ...
            rows.forEach(row => {
                ...
                const wtaRank = parseInt(cells[15].textContent.trim()) || null;
    ...
    getPlayerElo(playerName, surface = null) {
        ...
        if (!playerData) {
            console.warn(`WTA Elo rating not found for player: ${playerName}`);
            return null;
        }
    ...
    matchProbToHoldProb(matchWinProb, surface = 'Hard') {
        // Surface-specific base hold rates (slightly lower for WTA compared to ATP)
        const surfaceBases = {
            'Grass': 0.72,
            'Indoor': 0.70,
            'Hard': 0.65,
        ...
        const baseHold = surfaceBases[surface] || 0.65;
        ...
        // Underdog (0.2 match prob) ≈ 0.52 hold

        if (matchWinProb >= 0.5) {
            // Favorite: scale from base to 0.82
            const range = 0.82 - baseHold;
            return baseHold + (matchWinProb - 0.5) * 2 * range;
        } else {
            // Underdog: scale from 0.48 to base
            const range = baseHold - 0.48;
            return 0.48 + matchWinProb * 2 * range;
        }
    }
...
export const tennisWtaEloService = new TennisWtaEloService();
